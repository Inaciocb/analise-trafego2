...
int main(int argc, char *argv[])
{
  ...
  {
    std::cerr << "Uso: " << argv[0]
              << " arquivo_entrada.pcap arquivo_saida_mapa_calor.csv\n";
    return 1;
  }
  ...
  capturador.sniff_loop([&](const Packet &pacote)
        ...
        uint64_t indice_janela = segundo / TAMANHO_JANELA_SEGUNDOS;
  ...
  {
    uint64_t inicio_segundos = par_janela.first * TAMANHO_JANELA_SEGUNDOS;
    for (const auto &par_ip : par_janela.second)
    {
      saida << inicio_segundos
            << "," << par_ip.first
            << "," << par_ip.second
            << "\n";
    }
  ...
  std::cout << "CSV para mapa de calor gerado em " << arquivo_csv << "\n";
  return 0;
}
